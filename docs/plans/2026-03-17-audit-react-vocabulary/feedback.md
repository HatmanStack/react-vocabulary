...
| Integrate or remove `adaptiveDifficultyStore`                | NOT VERIFIED | Store still exists at `src/shared/store/adaptiveDifficultyStore.ts` but `getOptimalQuestionType` is never called by quizStore. The quiz flow generates fixed question types (line 103-107 of quizStore: always 1 multiple + 1 fillin per word). The store is disconnected from the quiz flow |
